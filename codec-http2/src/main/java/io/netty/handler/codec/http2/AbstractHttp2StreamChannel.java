...
abstract class AbstractHttp2StreamChannel extends DefaultAttributeMap implements Http2StreamChannel {
    ...
    private final ChannelFutureListener windowUpdateFrameListener = new ChannelFutureListener() {
        @Override
        public void operationComplete(ChannelFuture future) {
            handleWindowUpdateFrameFuture(future);
        }
    };
    ...
    private final class Http2ChannelUnsafe implements Unsafe {
        ...
        private void updateLocalWindowIfNeeded(int flowControlledBytes) {
            if (flowControlledBytes != 0) {
                ChannelFuture future = write0(parentContext(),
                        new DefaultHttp2WindowUpdateFrame(flowControlledBytes).stream(stream));
                writeDoneAndNoFlush = true;
                if (future.isDone()) {
                    handleWindowUpdateFrameFuture(future);
                } else {
                    future.addListener(windowUpdateFrameListener);
                }
            }
        }
        ...
        void doRead0(Http2Frame frame, RecvByteBufAllocator.Handle allocHandle) {
            ...
            if (frame instanceof Http2DataFrame) {
                ...
                updateLocalWindowIfNeeded(numBytesToBeConsumed);
            } else {
